...
router.post('/', isAuthenticated, (req, res) => {
    try {
        const { vehicle_id, service_id, part_number, part_name, expiry_date, installed_date } = req.body;
        ...
        const stmt = db.prepare(`
            INSERT INTO parts (vehicle_id, service_id, part_number, part_name, expiry_date, installed_date)
            VALUES (?, ?, ?, ?, ?, ?)
        `);
    ...
    try {
        ...
        const stmt = db.prepare(`
            UPDATE parts SET
                ...
                part_name = ?, expiry_date = ?, installed_date = ?
            WHERE id = ?
        `);
